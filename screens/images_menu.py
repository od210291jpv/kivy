...
class ImagesMenu(BaseScreen):
    def __init__(self, name, *navigate_screens):
        ...
        self.lower_grid = BoxLayout(orientation='vertical', spacing=5, padding=5)
        ...
        self.open_file_button = MDRaisedButton(text='Upload Image', size_hint=(1, .1))
        self.open_file_button.bind(on_release=lambda x: self.open_f(self.filechooser.path, self.filechooser.selection))

        self.lower_grid.add_widget(TextInput(size_hint=(1, .12), text='Image name'))
